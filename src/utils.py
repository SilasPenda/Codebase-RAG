...
# def get_embedding_model(model=embedding_model_name):
#     """
...
def get_embedding_model(model_name=embedding_model_name, model_dir="./models"):
    """
    Get a pre-trained embedding model using LangChain HuggingFaceEmbeddings.
    Downloads automatically if missing and caches locally.

    Args:
        model_name (str): Name of the embedding model.
        model_dir (str | Path): Directory to cache the model.

    Returns:
        HuggingFaceEmbeddings: Initialized embedding model.
    """
    model_dir = Path(model_dir) / model_name.replace("/", "_")
    model_dir.mkdir(parents=True, exist_ok=True)

    return HuggingFaceEmbeddings(model_name=model_name, cache_folder=str(model_dir))

def get_llm(type=llm_type, model_name=model_name):
    """
